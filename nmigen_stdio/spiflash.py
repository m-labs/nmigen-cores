...
class SPIFlash(Elaboratable):
    ...
    def _check_commands(self, cmd_dict):
        cmd_names = [
            "FAST_READ"
    ...
    def __init__(self, *, protocol, addr_width, data_width, cmd_width, cmd_dict, 
                 dummy_cycles=15, divisor=1, device=None, pins=None):
    ...
    def elaborate(self, platform):
        ...
        if self._pins is not None:
            ...
            if self._protocol == "extended":
                m.d.comb += [
                    MultiReg(self._pins.miso.i, self.miso),
                    self._pins.mosi.o.eq(self.mosi)
                ]
            elif self._protocol in ["dual", "quad"]:
                dq_oe = Signal()
                m.submodules.dq = platform.get_tristate(self.dq, self._pins.dq, None, False)
        # If the user doesn't give pins, create dq Pins
        else:
            dq_oe = Signal()
        ...
        if "FAST_READ" in self._cmd_dict:
            ...
            if self._protocol == "extended":
                # TODO: implement FAST READ for Extended SPI protocol as well
                pass
                ...
                with m.FSM() as fsm:
                    state_durations = {
                        ...
                        "FASTREAD-WAITREAD": self._divisor_val*(self.dummy_cycles+
